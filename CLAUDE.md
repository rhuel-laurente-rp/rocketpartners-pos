...
**The Total invariant.** Once **Total** is pressed the basket is finalized: `TransactionState` becomes `TOTALED` and further mutation is rejected by `Transaction` itself. Add-item, void-line, and change-quantity all throw. The actions legal in `TOTALED` are the tenders, Void Basket, and **Resume** (`Transaction.resumeEditing()` / the header "Add Item" control — the standard-POS recall/edit that moves `TOTALED` back to `IN_PROGRESS` so more lines can be rung up, clearing engine-applied discounts, which recompute at the next Total). Resume does **not** weaken the invariant: while the transaction is `TOTALED`, add-item/void-line/change-quantity still throw — the cashier must resume first. Enforced on the aggregate — disabling buttons is a nicety, not the guarantee.
