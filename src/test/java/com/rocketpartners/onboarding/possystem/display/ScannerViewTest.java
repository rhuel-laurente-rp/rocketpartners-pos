...
/**
 * Pure view tests for the scan bar's four modes: idle, focused (implicit via the border swap),
 * locked, and error. Runs headless — no real display required. Placeholder plumbing means the
 * field's document is "empty" (getScanText) whenever the placeholder is showing, so any
 * assertion that touches Scan-button gating must first put the field into a non-placeholder
 * state via {@link ScannerView#setScanText(String)}.
 */
class ScannerViewTest {
    ...
    @Test
    void scanButton_disabledWithEmptyField_enabledOncedigitsPresent() {
        assertThat(view.getScanButtonForTest().isEnabled()).isFalse();

        view.setScanText("049000053418");

        assertThat(view.getScanButtonForTest().isEnabled()).isTrue();
    ...
    @Test
    void scanButton_disabledWhenLocked_evenWithDigitsPresent() {
        view.setScanText("049000053418");
        assertThat(view.getScanButtonForTest().isEnabled()).isTrue();
        ...
        assertThat(view.getScanButtonForTest().isEnabled()).isFalse();
        // Field is non-editable when locked.
    ...
    @Test
    void scanButtonClick_dispatchesScanSubmit_withRawText() {
        view.setScanText("049000053418");

        view.getScanButtonForTest().doClick();

        assertThat(dispatcher.received)
                .extracting(PosEvent::getType)
                .contains(PosEventType.SCAN_SUBMIT_PRESSED);
    ...
    void clearScanField_whileFocused_doesNotPaintPlaceholderBackIntoField() {
        ...
        boolean okPlaceholder = "Scan or type a UPC and press Enter".equals(text);
