...
/**
 * A {@link Journal} that writes each entry to a local {@link PrintStream} (stdout by default).
 * Unconditionally-available: no network, no queue, no failure paths. Kept as its own class so
 * the POS always has a record even when the remote journal has never been reachable.
 */
