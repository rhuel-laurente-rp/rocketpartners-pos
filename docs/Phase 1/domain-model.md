...
- **`Discount` is a value on a Transaction, not a rule.** The rule that produced it (BOGO, "10% off produce", etc.) lives in the discount engine's database (Phase 3, live) as a `DiscountRule` entity. What the POS holds is the *result* of applying a rule, returned by the engine on `POST /discounts/calculate`: an amount and a description for the Receipt.
